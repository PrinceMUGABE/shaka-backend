...
def register_user(request):
    ...
    if not names:
        return Response({"error": "Names are required."}, status=400)
    ...
    try:
        ...
        if email:
            message = (
                "Hello,\n\nYour account has been created in AdvantConnect System.\n"
...
def login_user(request):
    email_or_phone = request.data.get('phone_number')
    password = request.data.get('password')
    
    print(f"Subitted login data: {request.data}\n\n")
...
def reset_password(request):
    ...
    try:
        ...
        send_mail(
            ...
            message=f"Your password has been reset to AdventConnect System.\n Your new password is: {new_password}",
            from_email="no-reply@agape.com",
...
def update_user(request, user_id):
    ...
    if not phone_number or not role or not names:
        return Response({"message": "names, Phone number, email, and role are required for updating a user."}, status=400)
...
def list_all_users(request):
    ...
    users = CustomUser.objects.all().values(
        'id', 'names', 'phone_number', 'email', 'role', 'status', 'created_at',
    )
...
def list_all_clients(request):
    ...
    print("User retrieving clients: ", request.user)
    # Retrieve only users with role 'client'
    users = CustomUser.objects.filter(role='client').values(
        'id', 'names', 'phone_number', 'email', 'role', 'status', 'created_at',
    )
